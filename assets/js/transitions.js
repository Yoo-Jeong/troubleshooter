/* ============================================================
   ...
   전환 스타일:  'accesslog' | 'scan-access' | 'scan' | 'slide-left' | 'slide-up' | 'resync' | 'fade' | 'instant'
   ------------------------------------------------------------
...
(function(){
  ...
  var CFG = TS.config = {
    ...
    log: ['> ACCESSING CHARACTER_DB','> AUTHORIZING · AGENT','> DECRYPTING RECORDS'],
    bootSteps: [
      'POWER ON · TS-TERM MDL-2','ESTABLISHING SECURE LINK',
      'AUTHENTICATING AGENT','HANDSHAKE · CENTRAL ARCHIVE','CHANNEL ENCRYPTED'
    ]
  };
  ...
  function seal(fx){                    // 이동 직전 스크림을 불투명하게 → 도착 커버로 매끈히 연결
    var s = fx && fx.querySelector('.scrim');
    if(s) s.style.background = 'var(--bg)';
  }
  ...
  TS.go = function(url, style, opts){
    ...
    runExit(style, fx, function(){ seal(fx); set(HANDOFF, JSON.stringify({style:style})); location.assign(url); });
  };
  ...
  function buildFx(style){
    ...
    fx.className = 'ts-fx';
    if(style === 'fade' || style === 'scan') fx.classList.add('solid');   // 나갈 땐 단색으로 덮음
    ...
    if(style === 'fade' || style === 'scan') fx.style.transition = 'opacity ' + CFG.fadeMs + 'ms ease';
  ...
  TS.boot = function(o){
    ...
    if(get(BOOTED) || reduce){               // 이미 이번 세션에 부팅함 → 스킵
    ...
    if(!host){
      ...
      host.innerHTML =
        '<div class="ct mono">TS·OS <b>HANDHELD ARCHIVE TERMINAL</b> v2.04</div>' +
